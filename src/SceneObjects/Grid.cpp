...
void
Grid::read_flat_triangles(string file_name) {
...
void
Grid::read_obj_file(string file_name, const int triangle_type) {
...
//				cout << "Before Line x: " << x << endl;
//				cout << "line length Before Line: " << line.length() << endl;
//				 do{
         	getline(myfile,line);					//we have the line
	//			}while(line.length() == 0);// && myfile.good());

//				 cout << "After Line x: " << x << endl;
//				 cout << "line length after Line: " << line.length() << endl;				 //Face
			 	//Vertex
				if(line.length() != 0){
...
//						std::size_t offset = 0;
//						x = std::stod(&xstring, &offset);
						stringstream xss(xstring);
						xss >> x;
...
//
 ...
 //						x = std::stod(&xstring, &offset);
	...
	//					 cout << "this is z: " << z << endl;
						 ...
						 faceList.push_back(z);
						 mesh_ptr->vertex_faces.push_back(faceList);
						 mesh_ptr->num_triangles++;


						 if (triangle_type == flat) {
		 			    FlatMeshTriangle* triangle_ptr = new FlatMeshTriangle(mesh_ptr, faceList[0], faceList[1], faceList[2]);
//							cout << "X: " << faceList[0] << " Y: " << faceList[1] << " Z: " << faceList[2] << endl;
...
//		cout << objects.size();
      ...
      cout << "File : " << file_name << " : did not open" << endl;
  }
//	cout << "num vertices: " << mesh_ptr->num_vertices << endl;
}
...
/*
	...
	char**			obj_info;
  	...
  	// open a ply file for reading
		cout << "about to open " << file_name << " for reading" << endl;
